    ...
    if PortList != []:
        str = "\n".join(
            [
                portArr.ljust(4)
                + "  "
                + (i[1] + min(len(i[1]), 1) * "  " + i[0]).ljust(36)
                + initial
                + " ;"
                for i in PortList
            ]
...
def formatPara(ParaList):
    ...
    if ParaList != []:
        ...
        paraDec = "\n".join(
            ["parameter %s = %s;" % (i[0].ljust(l1 + 1), i[1].ljust(l2)) for i in p]
        )
        paraDef = (
            "#(\n"
            + ",\n".join(
                ["    ." + i[0].ljust(l1 + 1) + "( " + i[0].ljust(l1) + " )" for i in p]
            )
            + ")\n"
        )
    else:
...
// finish
initial begin
    // $monitor("%d", xxx); // TODO Add monitor here
    #(PERIOD*300) $finish; // TODO Change the simulation time
end
"""
    print(operation)
    print("endmodule")
