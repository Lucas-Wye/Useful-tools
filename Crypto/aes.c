/*
   ./a.out 00112233445566778899aabbccddeeff 000102030405060708090a0b0c0d0e0f
*/
...
void Mix_Columns(unsigned char *state,int type){
  ...
  if(type == ENC){
    for(i = 0;i < 4;i++){
      for(j = 0;j < 4;j++){
        temp[i][j] = 
                     Mul(Mix_Mul[i][0], state[0 + j * 4])
                   ^ Mul(Mix_Mul[i][1], state[1 + j * 4])
                   ^ Mul(Mix_Mul[i][2], state[2 + j * 4])
                   ^ Mul(Mix_Mul[i][3], state[3 + j * 4]); 
      }
    }
  ...
  // 解密
    ...
    for(i = 0;i < 4;i++){
      for(j = 0;j < 4;j++){
        temp[i][j] = 
                     Mul(Mix_inv_Mul[i][0], state[0 + j * 4])
                   ^ Mul(Mix_inv_Mul[i][1], state[1 + j * 4])
                   ^ Mul(Mix_inv_Mul[i][2], state[2 + j * 4])
                   ^ Mul(Mix_inv_Mul[i][3], state[3 + j * 4]); 
      }
    }
